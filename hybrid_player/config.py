"""Configuration classes for hybrid player training and data processing."""
...
def get_base_dir():
    """Determine project root directory from current file location."""
...
@dataclass
class ModelConfig:
    """Configuration for language model and intent classifier training."""
    # Language model settings (DistilGPT-2)
    ...
    # Intent classifier settings (DistilBERT)
...
class HybridPlayerConfig:
    """Main configuration container for hybrid player system."""
    data: DataConfig = field(default_factory=DataConfig)
    model: ModelConfig = field(default_factory=ModelConfig)

    def __post_init__(self):
        """Log configuration paths for debugging purposes."""
        import logging
